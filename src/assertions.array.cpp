...
const f5::json::assertion::checker f5::json::assertion::min_items_checker = [](f5::u8view rule,
                                                                               f5::json::value part,
                                                                               f5::json::validation::annotations
                                                                                       an) {
    value array = an.data[an.dpos];
...
const f5::json::assertion::checker f5::json::assertion::unique_items_checker = [](f5::u8view rule,
                                                                                  f5::json::value part,
                                                                                  f5::json::validation::annotations
                                                                                          an) {
    value array = an.data[an.dpos];
    if (array.isarray()) {
        if (part == fostlib::json(true)) {
            ...
            throw fostlib::exceptions::not_implemented(
                    __func__, "unique items -- must be true or false", part);
        }
    }
    return validation::result{std::move(an)};
};
